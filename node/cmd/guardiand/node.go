...
func runNode(cmd *cobra.Command, args []string) {
	...
	supervisor.New(rootCtx, logger, func(ctx context.Context) error {
		...
		if shouldStart(ethRPC) {
			...
			ethWatcher = evm.NewEthWatcher(*ethRPC, ethContractAddr, "eth", common.ReadinessEthSyncing, vaa.ChainIDEthereum, lockC, setC, chainObsvReqC[vaa.ChainIDEthereum], *unsafeDevMode, nil)
		...
		if shouldStart(bscRPC) {
			...
			bscWatcher := evm.NewEthWatcher(*bscRPC, bscContractAddr, "bsc", common.ReadinessBSCSyncing, vaa.ChainIDBSC, lockC, nil, chainObsvReqC[vaa.ChainIDBSC], *unsafeDevMode, nil)
			bscWatcher.SetConfirmationParams(true, 60)
		...
		if shouldStart(polygonRPC) {
			...
			polygonWatcher := evm.NewEthWatcher(*polygonRPC, polygonContractAddr, "polygon", common.ReadinessPolygonSyncing, vaa.ChainIDPolygon, lockC, nil, chainObsvReqC[vaa.ChainIDPolygon], *unsafeDevMode, nil)
			polygonWatcher.SetConfirmationParams(waitForConfirmations, 4)
		...
		if shouldStart(avalancheRPC) {
			...
			if err := supervisor.Run(ctx, "avalanchewatch",
				evm.NewEthWatcher(*avalancheRPC, avalancheContractAddr, "avalanche", common.ReadinessAvalancheSyncing, vaa.ChainIDAvalanche, lockC, nil, chainObsvReqC[vaa.ChainIDAvalanche], *unsafeDevMode, nil).Run); err != nil {
				return err
			}
		}
		if shouldStart(oasisRPC) {
			...
			if err := supervisor.Run(ctx, "oasiswatch",
				evm.NewEthWatcher(*oasisRPC, oasisContractAddr, "oasis", common.ReadinessOasisSyncing, vaa.ChainIDOasis, lockC, nil, chainObsvReqC[vaa.ChainIDOasis], *unsafeDevMode, nil).Run); err != nil {
				return err
			}
		}
		if shouldStart(auroraRPC) {
			...
			if err := supervisor.Run(ctx, "aurorawatch",
				evm.NewEthWatcher(*auroraRPC, auroraContractAddr, "aurora", common.ReadinessAuroraSyncing, vaa.ChainIDAurora, lockC, nil, chainObsvReqC[vaa.ChainIDAurora], *unsafeDevMode, nil).Run); err != nil {
				return err
			}
		}
		if shouldStart(fantomRPC) {
			...
			if err := supervisor.Run(ctx, "fantomwatch",
				evm.NewEthWatcher(*fantomRPC, fantomContractAddr, "fantom", common.ReadinessFantomSyncing, vaa.ChainIDFantom, lockC, nil, chainObsvReqC[vaa.ChainIDFantom], *unsafeDevMode, nil).Run); err != nil {
				return err
			}
		}
		if shouldStart(karuraRPC) {
			...
			if err := supervisor.Run(ctx, "karurawatch",
				evm.NewEthWatcher(*karuraRPC, karuraContractAddr, "karura", common.ReadinessKaruraSyncing, vaa.ChainIDKarura, lockC, nil, chainObsvReqC[vaa.ChainIDKarura], *unsafeDevMode, nil).Run); err != nil {
				return err
			}
		}
		if shouldStart(acalaRPC) {
			...
			if err := supervisor.Run(ctx, "acalawatch",
				evm.NewEthWatcher(*acalaRPC, acalaContractAddr, "acala", common.ReadinessAcalaSyncing, vaa.ChainIDAcala, lockC, nil, chainObsvReqC[vaa.ChainIDAcala], *unsafeDevMode, nil).Run); err != nil {
				return err
			}
		}
		if shouldStart(klaytnRPC) {
			...
			if err := supervisor.Run(ctx, "klaytnwatch",
				evm.NewEthWatcher(*klaytnRPC, klaytnContractAddr, "klaytn", common.ReadinessKlaytnSyncing, vaa.ChainIDKlaytn, lockC, nil, chainObsvReqC[vaa.ChainIDKlaytn], *unsafeDevMode, nil).Run); err != nil {
				return err
			}
		}
		if shouldStart(celoRPC) {
			...
			if err := supervisor.Run(ctx, "celowatch",
				evm.NewEthWatcher(*celoRPC, celoContractAddr, "celo", common.ReadinessCeloSyncing, vaa.ChainIDCelo, lockC, nil, chainObsvReqC[vaa.ChainIDCelo], *unsafeDevMode, nil).Run); err != nil {
				return err
			}
		}
		if shouldStart(moonbeamRPC) {
			...
			if err := supervisor.Run(ctx, "moonbeamwatch",
				evm.NewEthWatcher(*moonbeamRPC, moonbeamContractAddr, "moonbeam", common.ReadinessMoonbeamSyncing, vaa.ChainIDMoonbeam, lockC, nil, chainObsvReqC[vaa.ChainIDMoonbeam], *unsafeDevMode, nil).Run); err != nil {
				return err
			}
		}
		if shouldStart(arbitrumRPC) {
			...
			if err := supervisor.Run(ctx, "arbitrumwatch",
				evm.NewEthWatcher(*arbitrumRPC, arbitrumContractAddr, "arbitrum", common.ReadinessArbitrumSyncing, vaa.ChainIDArbitrum, lockC, nil, chainObsvReqC[vaa.ChainIDArbitrum], *unsafeDevMode, ethWatcher).Run); err != nil {
				return err
			}
		}
		if shouldStart(optimismRPC) {
			...
			if err := supervisor.Run(ctx, "optimismwatch",
				evm.NewEthWatcher(*optimismRPC, optimismContractAddr, "optimism", common.ReadinessOptimismSyncing, vaa.ChainIDOptimism, lockC, nil, chainObsvReqC[vaa.ChainIDOptimism], *unsafeDevMode, ethWatcher).Run); err != nil {
				return err
			}
		}
		...
		if *testnetMode {
			if shouldStart(neonRPC) {
				...
				if err := supervisor.Run(ctx, "neonwatch",
					evm.NewEthWatcher(*neonRPC, neonContractAddr, "neon", common.ReadinessNeonSyncing, vaa.ChainIDNeon, lockC, nil, chainObsvReqC[vaa.ChainIDNeon], *unsafeDevMode, nil).Run); err != nil {
					return err
				}
			}
